...
class maps:
    ...
    def addpoint(self, lat, lng, color = '#FF0000', title=None, food=None):
        self.points.append((lat,lng,color[1:],title,food))
    ...
    def drawpoint(self,f,lat,lon,color,title,text):
        f.write('\t\tvar info = new google.maps.InfoWindow({content: "%s"});\n'%(text))
        ...
        f.write("""func = function(info, marker) {
            return function() {
                info.open(map, marker);
            };
        };
        google.maps.event.addListener(marker, "click", func(info, marker));
        """)
        f.write('\n')
